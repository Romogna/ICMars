...
Crr_cmd::Crr_cmd(void) 
{
	...
	pose_sub = n.subscribe("pose",1000,&Crr_cmd::poseCallback, this);
...
void Crr_cmd::poseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg)
{
  latestPose = *msg;
}
...
// TODO temp return here while working on restructuring cmds to JSON
return;
	...
	if(tokens[0] == "MT") {
		...
		ROS_INFO("Goto command (x,y,yaw)");
		geometry_msgs::Pose2D msg;
    float x,y,yaw;
		sscanf(tokens[1].c_str(), "%f", &x);
		sscanf(tokens[2].c_str(), "%f", &y);
		sscanf(tokens[3].c_str(), "%f", &yaw);
		msg.x = x;
		msg.y = y;
    msg.theta = yaw;
